...
- **規格化（Normalization）**：基準（例：$P=\frac{1}{2}$ の情報量を $1$ bit）でスケールを固定
...
これらの性質を同時に満たす関数は（定数倍を除けば）$-\log_2 P$ しかなく、**情報量の定義として妥当かつ自然**であることがわかります。
...
この理由から、情報量は $-\log_2 P$ で定義するのが最も自然であり、唯一の選択になります。
...
- **性質タブ（単調性スライダー）**：  
  - スライダーで $P \to 0$ に近づくと $I$ は急増。
  - スライダーを一番左に移動しても、最小値が $P=0.0001$ になり、$P=0$ にはならない。
- **エントロピー計算器**：  
  - 入力確率の合計をチェック。  
...
- **鍵のエントロピー**：
  - AES-128：$128$ bit（$2^{128} \approx 3.4 \times 10^{38}$ 通り）
  - AES-256：$256$ bit（$2^{256} \approx 1.2 \times 10^{77}$ 通り）
- **設計思想**：
...
- **鍵長とエントロピー**：
  - $2048$ bit 鍵：約 $112$ bit の安全性（対称鍵換算）
  - $4096$ bit 鍵：約 $140$ bit の安全性
- **情報理論的特徴**：
...
- 意味：$2^{4.7} \approx 26$ 通りの鍵 → 最大 $26$ 回、平均 $13$ 回の試行で破綻
...
|---------|------------------|------------------|--------------|------|
| シーザー暗号 | $\log_2(26) \approx 4.7$ bit | 最大 $26$ 回、平均 $13$ 回 | マイクロ秒〜数分 | 容易に破綻 |
