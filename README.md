...
**シーザー暗号の例**：
- H(鍵) = log₂(26) ≈ 4.7ビット
- 意味：2^4.7 ≈ 26通りの鍵 → 最大26回、平均13回の試行で破綻
- 現代のコンピューターなら**マイクロ秒単位**で完了
- 手作業でも**数分**で全数調査可能

**AES-128との比較**：
- H(鍵) = 128ビット
- 意味：2^128通りの鍵 → 平均2^127回の試行が必要
- 現在の技術では**宇宙の年齢より長時間**
...
| 暗号方式 | 鍵のエントロピー H(鍵) | 総当たり攻撃の試行回数 | 現実的な攻撃時間 | 結果 |
|---------|------------------|------------------|--------------|------|
| シーザー暗号 | log₂(26) ≈ 4.7ビット | 最大26回、平均13回 | マイクロ秒〜数分 | 容易に破綻 |
| エニグマ | log₂(10²³) ≈ 76ビット | 平均 ~2⁷⁵回 | 統計解析により短縮 | 実際には破綻 |
| AES-128 | log₂(2¹²⁸) = 128ビット | 平均 ~2¹²⁷回 | 宇宙年齢の10²⁰倍 | 実用的に安全 |
| RSA-2048 | 鍵長2048ビット | 素因数分解アルゴリズム | 現在は数十年レベル | まだ安全 |
| ワンタイムパッド | H(鍵) = H(平文) | 無限大（情報得られず） | 破綻不可能 | 理論的に完全 |
